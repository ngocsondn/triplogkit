...
**Bối cảnh (2026-07-09):** ngoài mục tiêu catch-up chung, roadmap này giờ gắn với mục tiêu cụ thể hơn — CV đã cập nhật liệt kê Swift Concurrency, SwiftData, SPM là kỹ năng chuyên môn, cần demo thật để chứng minh khi phỏng vấn. File narrative gốc `doc/iOS-Interview-Roadmap 2.md` đã được tách thành các bài chi tiết trong [`doc/learning/`](learning/README.md) — mỗi bài đi theo khung 5 phần (vấn đề cũ → refactor → đúc kết nhớ lâu → phỏng vấn → bằng chứng), xem [`learning/README.md`](learning/README.md) để biết cách đọc. File này chỉ giữ vai trò checklist tiến độ, mỗi mục link sang bài chi tiết tương ứng.

## 0. Debugging & Tooling refresher
- [x] Instruments (Leaks/Allocations) + retain-cycle demo — `AutoShuffleManager.swift` + `AutoShuffleDetailView.swift` (Timer capture `self` mạnh) → [chi tiết](learning/01-retain-cycle-instruments.md)
- [x] Áp fix retain cycle (`[weak self]` + `invalidate()` đúng chỗ) và re-profile để confirm instance count về 0 → [chi tiết](learning/01-retain-cycle-instruments.md)

## 1. Concurrency (ưu tiên #2)
- [x] async/await cơ bản (network call giả lập) — toy demo: `FunFactService.swift` + `AsyncFunFactView.swift` → [chi tiết](learning/03-async-await.md)
- [x] actor & Sendable — toy demo: `ScoreCounter.swift` + `ActorRaceDemoView.swift` → [chi tiết](learning/04-actor-sendable.md)
- [ ] Swift 6 strict concurrency mode (bật cho cả project, xem lỗi gì nổi lên) → [kế hoạch](learning/05-swift6-strict-concurrency.md)
- [ ] Tự trả lời được 4 câu hỏi phỏng vấn actor/async mà không cần xem lại note — câu hỏi có sẵn trong [learning/03](learning/03-async-await.md#-phỏng-vấn) + [learning/04](learning/04-actor-sendable.md#-phỏng-vấn)

## 2. SwiftUI state management (ưu tiên #1)
- [x] `@Observable` macro thay `ObservableObject` + `@Published` — candidate: refactor `AutoShuffleManager` → [chi tiết](learning/02-observable-macro.md)
- [x] `NavigationStack` (đã dùng trong `ContentView`, thay `NavigationView` cũ)
- [ ] `@Entry` macro cho `EnvironmentValues`

## 3. Data & Persistence
- [ ] SwiftData cơ bản (`@Model`) — ví dụ nhỏ, tách riêng khỏi mục dưới nếu muốn học trước khi ráp vào project phỏng vấn → [kế hoạch](learning/06-swiftdata-model-query.md)
- [ ] `@Model Checkpoint` (SwiftData) theo đúng spec trong [learning/06](learning/06-swiftdata-model-query.md)
- [ ] `@Query` load list checkpoint, tự động refresh UI
- [ ] Tự trả lời được 3 câu hỏi phỏng vấn SwiftData mà không cần xem lại note — xem [learning/06](learning/06-swiftdata-model-query.md#-phỏng-vấn)

## 4. Ngôn ngữ Swift
- [ ] Macros — tổng quan cách hoạt động (đứng sau `@Observable`/`@Model` đã dùng) → [kế hoạch](learning/07-swift-language-misc.md)
- [ ] Typed throws (`throws(MyError)`) → [kế hoạch](learning/07-swift-language-misc.md)
- [ ] `~Copyable` / noncopyable types (optional, nâng cao) → [kế hoạch](learning/07-swift-language-misc.md)

## 5. Testing (ưu tiên #3)
- [ ] Swift Testing framework (`@Test`, `#expect`) — viết lại vài test trong target Tests → [kế hoạch](learning/08-swift-testing.md)
...
## 7. Dự án phỏng vấn — SPM + ráp nối (chi tiết ở [learning/09](learning/09-spm-modularization.md))
...
- [ ] Tự trả lời được 3 câu hỏi phỏng vấn SPM mà không cần xem lại note — xem [learning/09](learning/09-spm-modularization.md#-phỏng-vấn)

## 8. Interview readiness — không phải code (chi tiết đầy đủ + script trong [learning/10](learning/10-interview-talking-points.md))
...
Xem đầy đủ trong [learning/10-interview-talking-points.md](learning/10-interview-talking-points.md). Tóm tắt ưu tiên: học thuộc 2 script ở mục 8 trước (gần như chắc chắn bị hỏi, không cần code mới) → actor/async cơ bản (mục 1) → SwiftData model + `@Query` cơ bản, bỏ qua migration (mục 3) → SPM chỉ cần thao tác tạo local package 1 lần, bỏ XCFramework nếu gấp (mục 7).
...
- 2026-07-09: Tạo folder [`doc/learning/`](learning/README.md) — tách mỗi chủ đề thành 1 bài theo khung cố định (vấn đề cũ → refactor/giải pháp mới → đúc kết/khẩu quyết để nhớ lâu → câu hỏi phỏng vấn chuẩn → bằng chứng trong project). Khôi phục lại nguyên văn 2 talking point (gap Native iOS, cơ chế xác thực bundle) và spec SwiftData/SPM từ `doc/iOS-Interview-Roadmap 2.md` (file gốc đã bị xoá khỏi working tree nhưng còn trong git history) vào [learning/10](learning/10-interview-talking-points.md) và [learning/06](learning/06-swiftdata-model-query.md)/[learning/09](learning/09-spm-modularization.md). File này (`LEARNING_PROGRESS.md`) giờ chỉ còn vai trò checklist, link sang từng bài chi tiết.
